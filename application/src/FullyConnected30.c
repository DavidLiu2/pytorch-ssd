...
void FullyConnected30(
  ...
  im2col = l1_buffer + 384;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+240;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 372);
  DMA_copy_bias.number_of_2d_copies = 1;
  DMA_copy_bias.number_of_1d_copies = 1;
  DMA_copy_bias.length_1d_copy = (uint16_t) 12;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 3 : 3;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*32/8;
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 12 : 12;
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 3 : 3;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 1, 1, 80, 1, 80,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 3, 1*1, 80, 1*1, 80, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 108);
    ...
    b = (uint8_t *) (l1_buffer + 372 + _i_nof_load*12);
    W = (uint8_t *) (l1_buffer + 108);
    y = (uint8_t *) (l1_buffer + 88);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 1, 1, 3, 1, 3, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 88);
