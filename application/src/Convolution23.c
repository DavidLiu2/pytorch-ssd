...
void Convolution23(
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 25928);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 2) ? 16 : 32;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 16 : 32;
      ...
      y_tile_size_nof = (_i_nof_load+1 == 2) ? 16 : 32;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*32/8;
      y_length_nof_byte = (_i_nof_load+1 == 2)   ? 64 : 128;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 16 : 32;
      W_tile_size_nif = (_i_nif_load+1 == 2) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*3*3;
      W_length_nif_byte = (_i_nif_load+1 == 2) ? 1 : 1;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 20, 20, 32, 20, 48,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 32, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 25616);
    ...
    b = (uint8_t *) (l1_buffer + 25928 + _i_nof_load*128);
    W = (uint8_t *) (l1_buffer + 25616);
    y = (uint8_t *) (l1_buffer + 12808);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 10, 10, 32, 10, 48, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 12808);
