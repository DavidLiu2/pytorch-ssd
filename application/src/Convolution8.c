...
void Convolution8(
  ...
  im2col = l1_buffer + 27256;
  volatile uint8_t *pwt_buffer;
  pwt_buffer = im2col + 312;
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 27064);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 40 : 40;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 40, 16, 40, 48,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 26896);
    ...
    b = (uint8_t *) (l1_buffer + 27064 + _i_nof_load*64);
    W = (uint8_t *) (l1_buffer + 26896);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 40, 16, 40, 48, 0, 0, 0, 0, 0, 0, 32);
