...
void Convolution12(
  ...
  DMA_copy_y.stride_2d = 3840;
  DMA_copy_y.stride_1d = 192;
  DMA_copy_y.dir = 0;
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+432;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 31544);
  DMA_copy_bias.number_of_2d_copies = 1;
  DMA_copy_bias.number_of_1d_copies = 1;
  DMA_copy_bias.length_1d_copy = (uint16_t) 192;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 3) ? 16 : 16;
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 17 : 25;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 40 : 40;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 3)   ? 16 : 16;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 8 : 12;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 20 : 20;
      y_tile_size_nof = (_i_nof_load+1 == 3) ? 16 : 16;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*32/8;
      y_length_nof_byte = (_i_nof_load+1 == 3)   ? 64 : 64;
      W_tile_size_nof = (_i_nof_load+1 == 3) ? 16 : 16;
      W_tile_size_nif = (_i_nif_load+1 == 3) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*3*3;
      W_length_nif_byte = (_i_nif_load+1 == 3) ? 1 : 1;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 25, 40, 16, 40, 48,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 16, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 31376);
    ...
    b = (uint8_t *) (l1_buffer + 31544 + _i_nof_load*64);
    W = (uint8_t *) (l1_buffer + 31376);
    y = (uint8_t *) (l1_buffer + 16008);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 12, 20, 16, 20, 48, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 16008);
