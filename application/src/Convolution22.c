...
void Convolution22(
  ...
  DMA_copy_W.number_of_2d_copies = 80;
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+5120;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 14376);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 8 : 8;
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 80 : 80;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*32/8;
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 320 : 320;
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 80 : 80;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 64, 8, 64,  -1, -1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 80, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 9232);
    ...
    b = (uint8_t *) (l1_buffer + 14376 + _i_nof_load*320);
    W = (uint8_t *) (l1_buffer + 9232);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 4, 4, 80, 4, 80, 0, 0, 0, 0, 0, 0, 32);
