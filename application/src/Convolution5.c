...
void Convolution5(
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 32184);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 7)   ? 9 : 13;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 7)   ? 4 : 6;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 13, 80, 16, 80, 48,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
    ...
    b = (uint8_t *) (l1_buffer + 32184 + _i_nof_load*64);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 6, 40, 16, 40, 48, 0, 0, 0, 0, 0, 0, 32);
