...
void Convolution15(
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+2048;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 26664);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 256 : 256;
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 64, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 8);
    ...
    b = (uint8_t *) (l1_buffer + 26664 + _i_nof_load*256);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 8, 64, 8, 64, 0, 0, 0, 0, 0, 0, 32);
