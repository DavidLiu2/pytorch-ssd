...
void Convolution62(
  ...
  DMA_copy_W.stride_2d = 32;
  DMA_copy_W.stride_1d = 32;
  DMA_copy_W.number_of_2d_copies = 128;
  ...
  im2col = l1_buffer + 18248;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+4096;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 17736);
  DMA_copy_bias.number_of_2d_copies = 1;
  DMA_copy_bias.number_of_1d_copies = 1;
  DMA_copy_bias.length_1d_copy = (uint16_t) 512;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 128 : 128;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*32/8;
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 512 : 512;
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 128 : 128;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 5, 5, 32, 5, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 128, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 13616);
    ...
    b = (uint8_t *) (l1_buffer + 17736 + _i_nof_load*512);
    W = (uint8_t *) (l1_buffer + 13616);
    y = (uint8_t *) (l1_buffer + 808);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 5, 5, 128, 5, 128, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 808);
