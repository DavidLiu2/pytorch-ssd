...
void ReluConvolution8(
  ...
  im2col = l1_buffer + 32952;
  volatile uint8_t *pwt_buffer;
  pwt_buffer = im2col + 840;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 32 : 32;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 32 : 32;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 32 : 32;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 32 : 32;
      y_tile_size_nof = (_i_nof_load+1 == 3) ? 16 : 16;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 3)   ? 16 : 16;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 32, 32, 16, 32, 48,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 32784);
    ...
    W = (uint8_t *) (l1_buffer + 32784);
    y = (uint8_t *) (l1_buffer + 16392);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 32, 32, 16, 32, 48, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 16392);
