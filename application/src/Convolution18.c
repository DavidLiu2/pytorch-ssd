...
void Convolution18(
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 32424);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 3)   ? 4 : 8;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 3)   ? 4 : 8;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 20, 8, 20, 8,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
    ...
    b = (uint8_t *) (l1_buffer + 32424 + _i_nof_load*192);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 20, 48, 20, 48, 0, 0, 0, 0, 0, 0, 32);
