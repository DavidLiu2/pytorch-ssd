...
void Convolution0(
  ...
  DMA_copy_W.stride_2d = 27;
  DMA_copy_W.stride_1d = 3;
  ...
  im2col = l1_buffer + 35887;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+216;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 35855);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 5)   ? 33 : 33;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 61 : 101;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 30 : 50;
      ...
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 3 : 3;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 33, 101, 3, 160, 3,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 3*3, 3, 3*3, 3, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 35615);
    ...
    b = (uint8_t *) (l1_buffer + 35855 + _i_nof_load*32);
    W = (uint8_t *) (l1_buffer + 35615);
    y = (uint8_t *) (l1_buffer + 10007);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 16, 50, 8, 80, 8, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 10007);
