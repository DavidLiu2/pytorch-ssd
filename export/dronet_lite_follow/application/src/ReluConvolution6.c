...
void ReluConvolution6(
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+6912;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 22368);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 17 : 17;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 5 : 29;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 8 : 8;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 2 : 14;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 29, 24, 32, 24,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 3*3, 24, 3*3, 24, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 15432);
    ...
    b = (uint8_t *) (l1_buffer + 22368 + _i_nof_load*128);
    W = (uint8_t *) (l1_buffer + 15432);
    y = (uint8_t *) (l1_buffer + 11840);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 14, 32, 16, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 11840);
