...
void Convolution3(
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 35320);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 31 : 31;
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 8 : 27;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 31, 27, 16, 64, 16,  -1, -1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 24, 1*1, 16, 1*1, 16, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 34912);
    ...
    b = (uint8_t *) (l1_buffer + 35320 + _i_nof_load*96);
    W = (uint8_t *) (l1_buffer + 34912);
    y = (uint8_t *) (l1_buffer + 13400);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 16, 14, 24, 32, 24, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 13400);
