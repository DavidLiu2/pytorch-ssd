...
// ULTRA_VERBOSE                  False
// verbose_log                    
// node                           <dory.Parsers.HW_node.HW_node object at 0x7d7570c1e220>
// sdk                            gap_sdk
// number_of_clusters             1
// optional_type                  8bit
// func_name                      ReluAddition4
// flag_DW                        0
// optional                       ReluAdd
// FLAG_BATCHNORM                 0
// has_bias                       0
...
// x_tile_size_h                  5
// x_tile_size_w                  32
// x_tile_size_byte               15360
// x_tile_size_nif_byte           96
// x_stride_w_byte                3072
// x_stride_c_byte                96
// y_h                            32
// y_w                            32
// y_data_size_byte               8
...
// y_tile_size_h                  5
// y_tile_size_w                  32
// y_tile_size_byte               3840
// y_stride_w_byte                768
// y_stride_c_byte                24
// y_tile_size_nof_byte           24
// tile_dim_h                     7
// tile_dim_w                     1
// tile_dim_nof                   1
// tile_dim_nif                   1
...
// l1_y_offset                    15368
// l1_x2_offset                   19216
// y_tile_size_nof_last           24
// y_tile_size_h_last             2
// y_tile_size_w_last             32
// y_length_nof_byte_last         24
// x_tile_size_nif_last           24
// x_tile_size_nif_byte_last      96
// x_tile_size_h_last             2
// x_tile_size_w_last             32


#include "ReluAddition4.h"
#include "pulp.h"
#include "pmsis.h"
...
void ReluAddition4(
  ...
  int iter;
  // tile loop nest
  for(iter=0; iter<1*7*1; iter++) {

    last_nof = (_i_nof_load+1 == 1) ? 1 : 0;
    last_nif = (_i_nof_load+1 == 1) ? 1 : 0;
    last_h = (_i_h_load+1 == 7) ? 1 : 0;
    last_w = (_i_w_load+1 == 1) ? 1 : 0;

    x_tile_size_nif = (last_nif) ? 24 : 24;
    x_tile_size_h   = (last_h)   ? 2 : 5;
    ...
    DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 5, 32, 24, 32, 24,  0, 0,0, 0, 0, 0, 32);
    ...
    DMA_copy_x2.ext = dory_get_tile_3d(l2_x2, _i_h_load, _i_w_load, _i_nif_load, 5, 32, 24, 32, 24,  0, 0,0, 0, 0, 0, 32);
    DMA_copy_x2.loc = (l1_buffer + 19216);
    ...
    y_tile_size_h   = (last_h)   ? 2 : 5;
    y_tile_size_w   = (last_w)   ? 32 : 32;

    x = (uint8_t *) (l1_buffer + 0);
    x2 = (uint8_t *) (l1_buffer + 19216);
    y = (uint8_t *) (l1_buffer + 15368);

    y_tile_size_nof = (last_nof) ? 24 : 24;
    y_tile_size_h   = (last_h)   ? 2 : 5;
    y_tile_size_w   = (last_w)   ? 32 : 32;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (last_nof)   ? 24 : 24;
    asm volatile("": : :"memory");
    ...
    DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 5, 32, 24, 32, 24, 0, 0, 0, 0, 0, 0, 8);
    DMA_copy_y.loc = (l1_buffer + 15368);
