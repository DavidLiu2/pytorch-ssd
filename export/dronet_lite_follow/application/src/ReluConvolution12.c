...
void ReluConvolution12(
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+20736;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 26920);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 8 : 8;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 48, 8, 48,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 48, 3*3, 48, 3*3, 48, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 6160);
    ...
    b = (uint8_t *) (l1_buffer + 26920 + _i_nof_load*192);
    W = (uint8_t *) (l1_buffer + 6160);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 8, 48, 8, 48, 0, 0, 0, 0, 0, 0, 8);
