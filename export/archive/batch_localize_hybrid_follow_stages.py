...
def summarize_anti_collapse(fp_values: list[float], quantized_values: list[float]) -> dict[str, Any]:
    ...
    if np.any(collapse_mask):
        collapsed_fraction = float(np.mean(np.abs(quant[collapse_mask]) < 0.25))
...
def _find_first_node_after_with_input(model: onnx.ModelProto, *, input_name: str, start: int, end: int):
    for index in range(start, end + 1):
        node = model.graph.node[index]
        if input_name in node.input:
            return index, node
...
def _find_last_node_in_range(model: onnx.ModelProto, *, start: int, end: int):
    for index in range(end, start - 1, -1):
        return index, model.graph.node[index]
    return None, None
...
def resolve_hybrid_follow_onnx_taps(model: onnx.ModelProto, final_output_name: str) -> dict[str, dict[str, Any]]:
    ...
    if conv1_node is None or head_node is None:
    ...
    pool_index, pool_node = _find_last_op_before(
        model,
        op_type="AveragePool",
        start=conv1_index,
        end=pool_post_index,
    ...
    pool_input_node = None
    if pool_node is not None:
        pool_input_index = output_to_index.get(pool_node.input[0])
        if pool_input_index is not None:
            pool_input_node = model.graph.node[pool_input_index]

    conv2_tap_index = conv2_index
    conv2_tap_node = conv2_node
    conv2_tap_resolution = "node input contains stage4.1.conv2.weight"
    conv2_tap_scale_source = "conv2_eps_out"
    conv2_operator_name = "stage4.1.conv2"
    ...
    add_operator_name = "stage4.1.add"
    add_pre_resolution = "last Add between stage4.1.conv2 and flatten input"
    add_post_resolution = "first Floor after stage4.1.add and before flatten input"

    relu1_search_end = (
        conv2_index
        if conv2_index is not None
        else (add_pre_index if add_pre_index is not None else pool_post_index)
    )
    relu1_index, relu1_node = _find_last_op_before(
        model,
        op_type="Clip",
        start=conv1_index,
        end=relu1_search_end,
    )
    ...
    if conv2_tap_node is None:
        conv2_tap_index = relu1_index
        conv2_tap_node = relu1_node
        conv2_tap_resolution = (
            "single-conv fallback: using the stage4.1.relu1 output as the terminal "
            "main-branch tensor before the residual-add/global-pool boundary"
        )
        conv2_tap_scale_source = "add_input0_eps"
        conv2_operator_name = "stage4.1.relu1"

    if add_pre_node is None:
        add_pre_index = conv2_tap_index
        add_pre_node = conv2_tap_node
        add_pre_resolution = (
            "no residual Add node present; using the last stage4.1 main-path tensor "
            "before the stage4 terminal activation/pool"
        )
        add_post_index = pool_input_index
        add_post_node = pool_input_node
        if add_post_node is None:
            add_post_index = add_pre_index
            add_post_node = add_pre_node
        add_post_resolution = (
            "no residual Add node present; using the stage4 terminal activation/input to global pool"
        )
        add_operator_name = (
            "stage4.1.out_relu"
            if add_post_node.op_type == "Clip" and conv2_operator_name != "stage4.1.relu1"
            else conv2_operator_name
        )
    else:
    ...
    return {
        ...
        "stage4_1_conv2": {
            "node_name": conv2_tap_node.name,
            "node_index": conv2_tap_index,
            "op_type": conv2_tap_node.op_type,
            "output_name": conv2_tap_node.output[0],
            "resolution": conv2_tap_resolution,
            "semantic_scale_source": conv2_tap_scale_source,
            "operator_name": conv2_operator_name,
        },
        "stage4_1_add_pre_requant": {
            ...
            "resolution": add_pre_resolution,
            "operator_name": add_operator_name,
        },
        "stage4_1_add_post_requant": {
            ...
            "resolution": add_post_resolution,
            "operator_name": add_operator_name,
        },
        "global_pool_post_requant": {
...
def semantic_tensor_for_id_probe(
    ...
    add_context = id_quant_context.get("stage4.1.add") or {}
    add_report = (id_add_audit.get("reports") or {}).get("stage4.1.add") or {}
    add_tensors = (id_add_audit.get("tensors") or {}).get("stage4.1.add") or {}
    ...
    if alias == "stage4_1_conv2":
        conv2_context = id_quant_context["stage4.1.conv2"]
        if conv2_context.get("module_class") == "Identity" or "stage4_1_conv2" not in id_probe["tensors"]:
            add_eps_in_list = (id_quant_context.get("stage4.1.add") or {}).get("eps_in_list") or []
            if add_eps_in_list and "stage4_1_add_input0" in id_probe["tensors"]:
                return np.asarray(id_probe["tensors"]["stage4_1_add_input0"], dtype=np.float64) * float(add_eps_in_list[0])
    ...
    if alias == "stage4_1_add_pre_requant":
        if "pre_requant_semantic" in add_tensors:
            return np.asarray(add_tensors["pre_requant_semantic"], dtype=np.float64)
        raw = np.asarray(
            id_probe["tensors"].get("stage4_1_add", id_probe["tensors"].get("stage4_1_conv2")),
            dtype=np.float64,
        )
        eps = add_context.get("eps_out")
        if eps is None:
            eps = (add_context.get("eps_in_list") or [None])[0]
        if eps is None:
            eps = (id_quant_context.get("stage4.1.conv2") or {}).get("eps_out")
        return raw * float(eps)
    if alias == "stage4_1_add_post_requant":
        if "post_requant_semantic" in add_tensors:
            return np.asarray(add_tensors["post_requant_semantic"], dtype=np.float64)
        raw = np.asarray(
            id_probe["tensors"].get("stage4_1_out_relu", id_probe["tensors"].get("stage4_1_add")),
            dtype=np.float64,
        )
        eps = add_context.get("eps_out")
        if eps is None:
...
def semantic_tensor_for_onnx_probe(
    ...
    if alias == "stage4_1_conv1":
        ...
        return raw * eps
    if alias == "stage4_1_conv2":
        tap_info = (onnx_probe.get("selected") or {}).get(alias) or {}
        if tap_info.get("semantic_scale_source") == "add_input0_eps":
            add_eps_in_list = (id_quant_context.get("stage4.1.add") or {}).get("eps_in_list") or []
            if add_eps_in_list:
                return raw * float(add_eps_in_list[0])
        eps = float(id_quant_context["stage4.1.conv2"]["eps_out"])
        return raw * eps
    if alias == "stage4_1_add_pre_requant":
        scale = add_report.get("scale_selection") or {}
        if scale.get("eps_out") is not None and scale.get("D") not in (None, 0):
    ...
    if alias == "stage4_1_add_post_requant":
        scale = add_report.get("scale_selection") or {}
        if scale.get("eps_out") is not None:
...
def semantic_tolerance_for_alias(
    ...
    if alias == "stage4_1_conv2":
        conv2_context = id_quant_context["stage4.1.conv2"]
        if conv2_context.get("module_class") == "Identity":
            add_eps_in_list = (id_quant_context.get("stage4.1.add") or {}).get("eps_in_list") or []
            if add_eps_in_list:
                return max(float(add_eps_in_list[0]) * 0.5, 1e-6)
    ...
    if alias == "stage4_1_add_pre_requant":
        ...
        if eps is None:
            eps = (add_context.get("eps_in_list") or [None])[0]
        return max(float(eps or 1e-6) * 0.5, 1e-6)
    if alias == "stage4_1_add_post_requant":
        scale = add_report.get("scale_selection") or {}
        if scale.get("eps_out") is not None:
...
def semanticize_focus_taps(
    ...
    conv2_context = id_quant_context.get("stage4.1.conv2") or {}
    add_tensors = (id_add_audit.get("tensors") or {}).get("stage4.1.add") or {}
    conv2_tensor_key = "stage4_1_conv2"
    if conv2_tensor_key not in fq_probe["tensors"] and "stage4_1_add_input0" in fq_probe["tensors"]:
        conv2_tensor_key = "stage4_1_add_input0"
    conv2_fq_tensor = np.asarray(fq_probe["tensors"][conv2_tensor_key], dtype=np.float64)
    if conv2_context.get("module_class") == "Identity" and "stage4_1_add_input0" in fq_probe["tensors"]:
        conv2_fq_tensor = np.asarray(fq_probe["tensors"]["stage4_1_add_input0"], dtype=np.float64)
    add_pre_fq = np.asarray(
        fq_probe["tensors"].get("stage4_1_add", fq_probe["tensors"].get(conv2_tensor_key)),
        dtype=np.float64,
    )
    add_post_fq = np.asarray(
        fq_probe["tensors"].get("stage4_1_out_relu", fq_probe["tensors"].get("stage4_1_add", fq_probe["tensors"].get(conv2_tensor_key))),
        dtype=np.float64,
    )
    if add_tensors.get("post_requant_semantic") is not None and "stage4_1_out_relu" not in fq_probe["tensors"]:
        add_post_fq = add_pre_fq
    fq = {
        "stage4_1_conv1": np.asarray(fq_probe["tensors"]["stage4_1_conv1"], dtype=np.float64),
        "stage4_1_conv2": conv2_fq_tensor,
        "stage4_1_add_pre_requant": add_pre_fq,
        "stage4_1_add_post_requant": add_post_fq,
...
def build_focus_tap_report(
    ...
    for alias in TAP_ALIAS_ORDER:
        ...
        point = {
            ...
            "operator_name": (onnx_probe["selected"][alias] or {}).get("operator_name"),
    ...
    return {
        ...
        "stage4_1_add_scale_selection": ((id_add_audit.get("reports") or {}).get("stage4.1.add") or {}).get("scale_selection"),
...
def summary_markdown(payload: dict[str, Any]) -> str:
    ...
    dominant_issue = payload.get("dominant_issue") or {}
    lines = [
    ...
    for stage_name in ("fq", "id", "onnx", "gvsoc"):
        ...
        lines.append(
            "- {}: sign_flip_rate={} corr={} slope={} collapsed_fraction={}".format(
                stage_name,
                "n/a" if stage_metrics.get("sign_flip_rate") is None else f"{float(stage_metrics['sign_flip_rate']):.3f}",
                "n/a" if stage_metrics.get("correlation") is None else f"{float(stage_metrics['correlation']):.3f}",
                "n/a" if stage_metrics.get("slope") is None else f"{float(stage_metrics['slope']):.3f}",
                "n/a" if stage_metrics.get("collapsed_fraction") is None else f"{float(stage_metrics['collapsed_fraction']):.3f}",
            )
        )
    ...
    lines.extend(
        [
            ...
            f"- Golden -> GVSOC threshold warnings: `{export_fidelity['golden_to_gvsoc_threshold_warn_count']}`",
            "",
    ...
    lines.extend(
        [
            "",
            "## Worst Golden -> GVSOC Drift",
            "",
            "| Image | Golden->GVSOC score | First failing transition |",
            "| --- | ---: | --- |",
        ]
    )
    for item in payload["rankings"]["golden_to_gvsoc"][:5]:
        lines.append(
            "| `{}` | `{:.6f}` | `{}` |".format(
                item["image_name"],
                float(item["scores"]["golden_to_gvsoc"]),
                item["first_material_transition"]["transition_label"] or "none",
            )
        )
...
def main() -> None:
    ...
    for row in batch_items:
        ...
        stage_results = {
            ...
            "gvsoc": make_stage_result(key="gvsoc", label="Application/GVSOC", stage_tag="id", representation="fixed-point-int32", raw_native=sample_artifacts["gvsoc"]),
        }
        stage_results["application"] = stage_results["gvsoc"]
        ...
        pairwise = {
            ...
            "onnx_to_gvsoc": compare_stage_pair(stage_results["onnx"], stage_results["gvsoc"], thresholds),
            "onnx_to_application": compare_stage_pair(stage_results["onnx"], stage_results["gvsoc"], thresholds),
    ...
    golden_to_gvsoc_ranking = sorted(
        per_image_records,
        key=lambda item: float(item["scores"]["golden_to_gvsoc"] if item["scores"]["golden_to_gvsoc"] is not None else -1.0),
        reverse=True,
    )
    ...
    fp_x_values = [float(record["final_outputs"]["fp"]["decoded"]["x_offset"]) for record in per_image_records]
    anti_collapse = {
        "fq": summarize_anti_collapse(
            fp_x_values,
            [float(record["final_outputs"]["fq"]["decoded"]["x_offset"]) for record in per_image_records],
        ),
        "id": summarize_anti_collapse(
            fp_x_values,
            [float(record["final_outputs"]["id"]["decoded"]["x_offset"]) for record in per_image_records],
        ),
        "onnx": summarize_anti_collapse(
            fp_x_values,
            [float(record["final_outputs"]["onnx"]["decoded"]["x_offset"]) for record in per_image_records],
        ),
        "gvsoc": summarize_anti_collapse(
            fp_x_values,
            [float(record["final_outputs"]["gvsoc"]["decoded"]["x_offset"]) for record in per_image_records],
        ),
    }
    ...
    best_rank = (-1, -1.0)
    for bucket_name, count in transition_counts.items():
        severity_values = [
            float(pair_score(record["pairwise"].get(record["first_material_transition"]["transition_key"])) or 0.0)
            for record in per_image_records
            if record["first_material_transition"]["transition_label"] == bucket_name
        ]
        mean_severity = float(np.mean(severity_values)) if severity_values else None
        dominant_issue["buckets"][bucket_name] = {
            "count": int(count),
            "mean_severity": mean_severity,
        }
        if bucket_name == "none":
            continue
        rank = (int(count), float(mean_severity or 0.0))
        if rank > best_rank:
            best_rank = rank
            dominant_issue["dominant_bucket"] = bucket_name
    if best_rank[0] <= 0:
        dominant_issue["dominant_bucket"] = "none"
    ...
    export_fidelity = {
        ...
        "golden_to_gvsoc_threshold_warn_count": sum(
            1 for record in per_image_records if record["pairwise"]["golden_to_gvsoc"]["status"] == "warn"
        ),
    }
    ...
    result_payload = {
        ...
        "rankings": {
            ...
            "golden_to_gvsoc": golden_to_gvsoc_ranking,
        },
        "focus_images": [
            {
    ...
    write_json(output_dir / "ranked_golden_to_gvsoc.json", golden_to_gvsoc_ranking)
